...
//		this.status("keyCode=" + ev.keyCode + ' charCode=' + ev.charCode + ' detail=' + ev.detail);
        ...
        if(ev.keyCode >= 48 && ev.keyCode <= 57){
            var index = (ev.keyCode - 39) % 10; // Normalize 0-9, setting 1 to 0, 2 to 1, ..., and 0 to 9
            var rikaiObj = document.getElementById("rikaichan-window");
            var str = String.fromCharCode(ev.keyCode);
            console.log("Test "+str+" "+rikaiObj);
            console.log(rikaiObj);
            console.log(document.getElementsByClassName("w-kanji"));
            console.log(index);
            return;
        }
		switch (ev.keyCode) {
        ...
        case 82:
                var rikaiObj = document.getElementById("rikaichan-window");
            console.log(rikaiObj.innerText);
            break;
		case 89:	// y
			this.altView = 0;
